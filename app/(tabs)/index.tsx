...
const isDepositoCreditosAllowedForPerfil = (idPerfil) =>
  ![ROLE_IDS.PROVIDER, ROLE_IDS.BUSINESS_MANAGER].includes(
    Number(idPerfil ?? 0),
  );
        ...
        return (
          <TouchableOpacity
            ...
            style={[
              ...
              isActive && styles.timeWheelItemActive,
            ]}
          ...
          >
            <Text
              style={[
                ...
                isActive && styles.timeWheelItemTextActive,
              ]}
            >
              {pad2(value)}
            </Text>
    ...
    <View style={styles.dateTimeSelector}>
      <View style={styles.dateTimeHeader}>
        ...
        <Text style={styles.dateTimeValue}>
          {formatDateTimeValue(selectedDate)}
        </Text>
      </View>
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                ...
                isActive && styles.calendarDayChipActive,
              ]}
      ...
      <View style={styles.chronometerPanel}>
        ...
        <Text style={styles.chronometerValue}>
          {pad2(selectedDate.getHours())}:{pad2(selectedDate.getMinutes())}:
          {pad2(selectedDate.getSeconds())}
        </Text>
        <Text style={styles.chronometerFormat}>
          {formatDateTimeValue(selectedDate)}
        </Text>
      </View>
...
export default function HomeScreen() {
  ...
  const openDepositModal = async (targetUser) => {
    ...
    try {
      ...
      Alert.alert(
        "Atenci\u00f3n",
        error.message || "No se pudieron consultar los datos del deposito.",
      );
    } finally {
      setLoadingDepositDetails(false);
    }
  ...
  const handleGenerateUserQr = async (targetUser) => {
    ...
    try {
      ...
      Alert.alert(
        "Atenci\u00f3n",
        error.message || "No se pudo generar el QR del usuario.",
      );
    }
  };
  ...
  const handleDepositCredits = async () => {
    ...
    const amountValue =
      trimmedAmount === "" ? 0 : Number.parseFloat(trimmedAmount);
    const hasAmount =
      trimmedAmount !== "" && !Number.isNaN(amountValue) && amountValue > 0;
    const hasAnyVigencia = Boolean(
      depositVigenteDesde.trim() || depositVigenteHasta.trim(),
    );
    const hasQrDisponible = Boolean(
      depositQrRowId || String(depositQrCode ?? "").trim(),
    );
    ...
    if (
      ...
      Alert.alert(
        "Atenci\u00f3n",
        "Captura un monto valido para depositar creditos.",
      );
      return;
    }
    ...
    if (!hasAmount && !hasAnyVigencia) {
      Alert.alert(
        "Atenci\u00f3n",
        "Captura un monto o al menos una vigencia para guardar.",
      );
      return;
    }
    ...
    if (
      ...
      parseDateTimeInput(depositVigenteHasta).getTime() <
        parseDateTimeInput(depositVigenteDesde).getTime()
    ) {
      Alert.alert(
        "Atenci\u00f3n",
        "La vigencia final del QR no puede ser menor a la inicial.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await saveDepositoCredito({
        ...
        monto_deposito: trimmedAmount === "" ? "" : amountValue.toFixed(2),
        vigente_desde: vigenteDesdeSql,
        vigente_hasta: vigenteHastaSql,
      });

      closeDepositModal();
      await Promise.all([loadUsersView(), loadPaymentsView()]);
      Alert.alert(
        response?.data?.solo_vigencia
          ? "Vigencia actualizada"
          : "Creditos depositados",
        response?.respuesta ||
          response?.message ||
          "El deposito fue registrado correctamente.",
      );
    } catch (error) {
      console.error("Error depositing credits:", error);
      Alert.alert(
        "Atenci\u00f3n",
        error.message || "No se pudo registrar el deposito.",
      );
    } finally {
      setSavingDeposit(false);
    }
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  savingDeposit && styles.disabledButton,
                ]}
