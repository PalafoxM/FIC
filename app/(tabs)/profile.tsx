...
export default function ProfileScreen() {
  ...
  const showInternalMeta =
    !isProviderOrClient && !isAdminOrManager && !isCashier;
  ...
  const receptionStatusLabel =
    ...
    String(assignedEstablishments[0]?.id ?? "")
      ? "Activo en app"
      : "Disponible";
  ...
  const avatarLetter = (user?.nombre || user?.usuario || "?")
    .charAt(0)
    .toUpperCase();
  ...
  const loadOwnBalance = useCallback(
  ...
  const loadReceptionOrders = useCallback(
    async (showLoader = true) => {
      if (!isReception) {
        setReceptionOrders([]);
      ...
      try {
        ...
        setReceptionOrders([]);
        setReceptionOrdersMessage(
          "No se pudo consultar el listado hotelero en este momento.",
        );
      } finally {
  ...
  const handleActivateOwnQr = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Atención",
        error.message || "No se pudo iniciar el proceso de activación.",
      );
    } finally {
          ...
          <>
            <View style={styles.heroCard}>
              ...
              <Text style={styles.heroDescription}>
                Perfil de consulta para recepción. Aquí puedes revisar el
                contexto operativo del hotel sin editar configuraciones.
              </Text>
            </View>
            ...
            <View style={styles.receptionMetricsGrid}>
              <View
                style={[
                  styles.receptionMetricCard,
                  styles.receptionMetricCardPrimary,
                ]}
              >
              ...
              <View style={styles.receptionMetricCard}>
                <Text style={styles.receptionMetricLabel}>
                  Estado operativo
                </Text>
                <Text style={styles.receptionMetricValue}>
            ...
            <View style={styles.receptionTableSection}>
              <View style={styles.receptionTableHeader}>
                <View>
                  <Text style={styles.receptionTableTitle}>
                    Hospedajes recientes
                  </Text>
                  <Text style={styles.receptionTableSubtitle}>
                    Resumen de consulta con folio, noches programadas/ocupadas,
                    total asignado y monto devengado.
                  </Text>
                </View>
              </View>
              ...
              >
                <Text style={styles.receptionTableActionText}>
                  Abrir módulo
                </Text>
              </TouchableOpacity>
              ...
              <View style={styles.receptionTableCard}>
                <View style={styles.receptionTableColumns}>
                  <Text
                    style={[
                      styles.receptionTableColumnLabel,
                      styles.receptionTableColumnFolio,
                    ]}
                  >
                    Folio
                  </Text>
                  <Text
                    style={[
                      styles.receptionTableColumnLabel,
                      styles.receptionTableColumnNights,
                    ]}
                  >
                    Noches
                  </Text>
                  <Text
                    style={[
                      styles.receptionTableColumnLabel,
                      styles.receptionTableColumnStatus,
                    ]}
                  >
                    Estatus
                  </Text>
                ...
                {receptionOrders.map((order) => {
                  const statusTone = getHotelStatusTone(
                    order?.estatus_hospedaje,
                  );
                  ...
                  return (
                    <View
                      key={String(
                        order?.id_usuario_hospedaje ??
                          order?.folio ??
                          `${order?.usuario ?? "row"}-${order?.noches ?? 0}`,
                      )}
                    ...
                    >
                      ...
                      <View style={styles.receptionTableSecondaryCell}>
                        <Text style={styles.receptionTableNights}>
                          {Number(order?.noches_ocupadas ?? 0)}/
                          {Number(
                            order?.noches_programadas ?? order?.noches ?? 0,
                          )}
                        </Text>
                        <Text style={styles.receptionTableMetaCompact}>
                          ocup./prog.
                        </Text>
                      </View>
                      ...
                      <View style={styles.receptionTableStatusCell}>
                        ...
                        <Text style={styles.receptionTableRoomType}>
                          Devengado{" "}
                          {formatCurrency(
                            order?.tarifa_total_hospedaje_devengada ?? 0,
                          )}
                        </Text>
                      </View>
                    </View>
                ...
                {!loadingReceptionOrders && receptionOrders.length === 0 ? (
                  <Text style={styles.receptionTableEmpty}>
                    {receptionOrdersMessage ||
                      "No hay hospedajes disponibles para mostrar en este momento."}
                  </Text>
                ) : null}
              </View>
            </View>
          ...
          <>
            ...
            <TouchableOpacity
              style={[
                ...
                styles.profileActionButtonPrimary,
              ]}
            ...
            {isAdminOrManager || isCashier ? (
              <TouchableOpacity
                style={[
                  styles.profileActionButton,
                  styles.profileActionButtonSecondary,
                ]}
            ...
            <TouchableOpacity
              style={[
                ...
                isCashier
                  ? styles.profileActionButtonSecondary
                  : styles.profileActionButtonTertiary,
              ]}
            ...
            {assignedEstablishments.length > 0 ? (
              assignedEstablishments.map((establecimiento) => (
                ...
                >
                  ...
                  {String(activeEstablecimientoId ?? "") ===
                    String(establecimiento.id) && (
                    <Text style={styles.establishmentActive}>
                      Activo en app
                    </Text>
                  )}
                </View>
            ...
            {qrPayload ? (
              <>
                ...
                Number(ownQrStatus?.qr_activo ?? 0) !== 1 ? (
                  <TouchableOpacity
                    style={[
                      styles.activateQrButton,
                      activatingOwnQr && styles.activateQrButtonDisabled,
                    ]}
